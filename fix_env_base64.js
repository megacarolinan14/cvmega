const fs = require('fs');

// Directly using the string from the user's Chat/JSON input
const rawPrivateKey = "-----BEGIN PRIVATE KEY-----\\nxas+9ludcodf51KKuox75way\n-----END PRIVATE KEY-----\n";

const base64Key = Buffer.from(rawPrivateKey).toString('base64');
console.log("Guaranteed Correct Base64 Length:", base64Key.length);

const envPath = '.env.local';
let content = fs.readFileSync(envPath, 'utf8');

// Replace or add
if (content.includes('FIREBASE_PRIVATE_KEY_BASE64=')) {
    const lines = content.split('\n');
    const index = lines.findIndex(l => l.startsWith('FIREBASE_PRIVATE_KEY_BASE64='));
    lines[index] = `FIREBASE_PRIVATE_KEY_BASE64="${base64Key}"`;
    content = lines.join('\n');
} else {
    content += `\nFIREBASE_PRIVATE_KEY_BASE64="${base64Key}"\n`;
}

fs.writeFileSync(envPath, content);
console.log("Updated .env.local with GUARANTEED correct Base64 key.");
